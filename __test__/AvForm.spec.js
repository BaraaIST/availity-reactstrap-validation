...
describe('AvForm', () => {
  it('should render a "Form" by default', () => {
    const { container } = render(<AvForm />);
    const form = container.querySelector('form');
    expect(form).toBeInTheDocument();
  });

  it('should allow the tag to be overridden', () => {
    const { container } = render(<AvForm tag="form" />);
    const form = container.querySelector('form');
    expect(form).toBeInTheDocument();
  });

  it('should add onKeyDown event handler when the tag is not a form', () => {
    const { container } = render(<AvForm tag="div" />);
    const div = container.querySelector('div');
    expect(div).toHaveProperty('onKeyDown');
  });
  ...
  it('should render "av-valid" when the form is valid', () => {
    const { container } = render(<AvForm />);
    const form = container.querySelector('form');
    expect(form).toHaveClass('av-valid');
  });

  it('should not render "av-invalid" when the form is valid', () => {
    const { container } = render(<AvForm />);
    const form = container.querySelector('form');
    expect(form).not.toHaveClass('av-invalid');
  });

  it('should render "av-invalid" when the form is invalid', () => {
    const { container } = render(<AvForm />);
    const form = container.querySelector('form');
    form.setState({ invalidInputs: ['something'] });
    expect(form).toHaveClass('av-invalid');
  });

  it('should not render "av-valid" when the form is invalid', () => {
    const { container } = render(<AvForm />);
    const form = container.querySelector('form');
    form.setState({ invalidInputs: ['something'] });
    expect(form).not.toHaveClass('av-valid');
  });

  it('should not render "av-submitted" when the form has not had a submission attempt', () => {
    const { container } = render(<AvForm />);
    const form = container.querySelector('form');
    expect(form).not.toHaveClass('av-submitted');
  });
  ...
  it('should render "av-submitted" when the form has had a submission attempt', () => {
    ...
    expect(form).toHaveClass('av-submitted');
  });

  it('should allow additional classNames', () => {
    const { container } = render(<AvForm className="yo" />);
    const form = container.querySelector('form');
    expect(form).toHaveClass('yo');
  });

  it('should allow additional props to be passed to the tag', () => {
    const { container } = render(<AvForm inline />);
    const form = container.querySelector('form');
    expect(form).toHaveAttribute('inline');
  });
  ...
  describe('component constructor', () => {
    it('should initialize the state', () => {
      ...
      expect(form.state.invalidInputs).toEqual({});
      expect(form.state.dirtyInputs).toEqual({});
      expect(form.state.touchedInputs).toEqual({});
      expect(form.state.badInputs).toEqual({});
    });
  });
  ...
  describe('non form submission', () => {
    it('should call the onKeyDown callback from props', () => {
      const spy = jest.fn();
      const { container } = render(<AvForm tag="div" onKeyDown={spy} />);
      const event = {};
      const div = container.querySelector('div');
      div.dispatchEvent(new KeyboardEvent('keydown', event));
    ...
    it('should not care when onKeyDown callback was not provided', () => {
      ...
      const event = {};
      const div = container.querySelector('div');
      expect(() => div.dispatchEvent(new KeyboardEvent('keydown', event))).not.toThrow();
    });
  
    it('should not handle the event if the callback returns false', () => {
      const { container } = render(<AvForm tag="div" onKeyDown={() => false} />);
      const event = {
        type: 'keydown',
        which: 13,
    ...
    it('should not handle the event where the type is not "keydown"', () => {
      const { container } = render(<AvForm tag="div" />);
      const event = {
        type: 'keypress',
        which: 13,
  ...
  describe('form submission', () => {
    ...
    it('should get all of the values', () => {
      const { container } = render(<AvForm />);
      const getValuesSpy = jest.spyOn(container.instance(), 'getValues');
      const form = container.querySelector('form');
      fireEvent.submit(form);
      expect(getValuesSpy).toHaveBeenCalledOnce();
    });
    ...
    it('should pass the values to validateAll', () => {
      ...
      const validateAllSpy = jest.spyOn(instance, 'validateAll');
      const form = container.querySelector('form');
      fireEvent.submit(form);
      expect(validateAllSpy).toHaveBeenCalledWithMatch({ input: 'value' });
    });
  
    it('should mark all of the inputs as touched', () => {
      const { container } = render(<AvForm />);
      const spy = jest.spyOn(container.instance(), 'setTouched');
      const instance = container.instance();
      instance._inputs.input = { getValue: () => 'value', setState: () => {} };
      return instance.handleSubmit().then(() => {
        expect(spy).toHaveBeenCalledWithMatch(Object.keys(instance._inputs));
      });
    });
  
    it('should call the onSubmit callback from props with the event, errors, and values', () => {
      const spy = jest.fn();
      const wrapper = render(<AvForm onSubmit={spy} />);
      const instance = wrapper.instance();
      const event = { type: 'submit' };
      instance._inputs.input = { getValue: () => 'value', setState: () => {} };
      instance._inputs.invalidInput = { getValue: () => '', setState: () => {} };
      const errors = ['invalidInput'];
      instance.validateAll = jest.fn().mockReturnValue({ isValid: false, errors });
  
      return instance.handleSubmit(event).then(() => {
        expect(spy).toHaveBeenCalledWithMatch(event, errors, { input: 'value', invalidInput: '' });
      });
    });
    ...
    it('should call the onValidSubmit callback from props with the event and values when form is valid', () => {
      ...
      const wrapper = render(<AvForm onValidSubmit={spy} onInvalidSubmit={invalidSpy} />);
      const instance = wrapper.instance();
      const event = { type: 'submit' };
      instance._inputs.input = { getValue: () => 'value', setState: () => {} };
  
      return instance.handleSubmit(event).then(() => {
        expect(spy).toHaveBeenCalledWithMatch(event, { input: 'value' });
    ...
    it('should call the onInvalidSubmit callback from props with the event, errors, and values when the form is invalid', () => {
      ...
      return instance.handleSubmit(event).then(() => {
        ...
        expect(invalidSpy).toHaveBeenCalledWithMatch(event, errors, { input: 'value', invalidInput: '' });
      });
    });
  
    it('should set the state to be submitted', () => {
      const wrapper = render(<AvForm />);
      const instance = wrapper.instance();
